...
var Channel = class _Channel {
  ...
  fetchRelatedChannels = async (instance, opts = {
    ...
    const res = await import_got.default.get(queryURL);
    ...
    try {
      ...
      if (err instanceof import_got.HTTPError) {
...
var Instance = class {
  ...
  async fetchStats() {
    let stats;
    try {
      const res = await import_got2.default.get(`${this.url}/api/v1/stats`, {
      ...
      if (err instanceof import_got2.HTTPError) {
        if (err.message.includes("500"))
          throw new ServerError("Internal Server Error");
        else throw new APIError(err.message);
      } else throw new UnknownError(err.message);
...
// index.ts
var import_got3 = __toESM(require("got"));
var import_stream = require("stream");
var import_rereadable_stream = require("rereadable-stream");
...
var fetchInstances = async (opts = {
  ...
  const instances = [];
  try {
    const res = await import_got3.default.get("https://api.invidious.io/instances.json", {
    ...
    if (err instanceof import_got3.HTTPError) {
      if (err.message.includes("500"))
        throw new ServerError("Internal Server Error");
      else throw new APIError(err.message);
    } else throw new UnknownError(err.message);
...
var getInstance = async (url) => {
  try {
    let api = void 0;
    const stats = await import_got3.default.get(`${url}/api/v1/stats`, {
      headers: { "User-Agent": useragent3 }
    });
    ...
    const res = await import_got3.default.get(`${url}/api/v1`, {
      headers: { "User-Agent": useragent3 }
    });
...
var fetchVideo = async (instance, id, opts = {
  ...
  const searchParams = params.createQuery();
  try {
    const res = await import_got3.default.get(queryURL, {
      searchParams,
      headers: { "User-Agent": useragent3 }
    });
    const json = await JSON.parse(res.body);
...
var saveBlob = async (instance, video, source, opts = {
  ...
  const searchParams = params.createQuery();
  let length = 0;
  await import_got3.default.get(queryURL, {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    positions.forEach((position) => {
      ...
      promises.push(
        import_got3.default.get(queryURL, {
...
var saveStream = async (instance, video, source) => {
  ...
  return new Promise((resolve, reject) => {
    const stream = import_got3.default.stream(queryURL, {
      searchParams,
      headers: { "User-Agent": useragent3 },
      agent: {
        https: new import_https.default.Agent({ keepAlive: true })
      },
      http2: true
    });
    ...
    const result = new import_rereadable_stream.ReReadable();
